...
char *client_path() {
  ...
  if (result == ERROR_SUCCESS && buf_len > 0) {
    snprintf(full_path, 2048, "%s\\FreeTrackClient.dll", path);
  }
  RegCloseKey(hkey);
  return full_path;
}
...
bool start(HWND hwnd) {
  ...
  getDllVersion =
      (importGetDllVersion)(void *)GetProcAddress(ftclient, "FTGetDllVersion");
  reportName =
...
VOID CALLBACK TimerProcedure(HWND hwnd, UINT uMsg, UINT_PTR idEvent,
                             DWORD dwTime) {
  (void)uMsg;
  (void)idEvent;
  (void)dwTime;
...
BOOL CALLBACK DialogProc(HWND hwndDlg, UINT uMsg, WPARAM wParam,
  ...
  case WM_COMMAND:
    ...
    case IDC_START:
      start(hwndDlg);
      // l                  int ok;
      //                   int num = GetDlgItemInt(hwndDlg, IDC_APPID,
      //                   (BOOL*)&ok, false);
      if (timer != 0) {
