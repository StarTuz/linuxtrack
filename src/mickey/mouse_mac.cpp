...
mouseClass::mouseClass() { data = new mouseLocalData(); }

mouseClass::~mouseClass() { delete data; }

bool mouseClass::init() { return true; }

bool mouseClass::move(int dx, int dy) {
  // Gotcha - when a key is pressed, while moving, emit not mouse moved, but
  // mouse dragged event
  CGEventType event;
  CGEventRef ev_ref;
  CGPoint pos;
  ...
  ev_ref = CGEventCreateMouseEvent(nullptr, event, pos, 0);
...
bool mouseClass::click(buttons_t buttons, struct timeval ts) {
  ...
  if (chaged & RIGHT_BUTTON) {
    ...
    CGEventCreateMouseEvent(nullptr, event, pos, 0);
    CGEventPost(kCGHIDEventTap, ev_ref);
    CFRelease(ev_ref);
  }
